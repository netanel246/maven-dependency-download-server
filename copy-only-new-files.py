...
def initialize_internals():
	...
	output = subprocess.check_output(r'mvn dependency:go-offline -s {}'.format(settings_path), cwd=internals_path, shell=True)
	print(output)
...
def initialize_users_dependenies_dir(internals_path, users_pom_path):
	...
	output = subprocess.check_output(r'mvn dependency:go-offline -s {}'.format(temp_settings_path), cwd=temp_dir, shell=True)
	print(output)
